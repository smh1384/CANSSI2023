...
def apply_pca_and_cross_validation(X_train, y_train, X_val, y_val, best_model, n_components=PCA_COMPONENTS,
    ...
    return model_performance, cv_results, pipeline, final_estimator
...
def save_forecast_to_parquet(val_df, predictions, file_path):
    # Add the predictions to the validation dataframe
    val_df['forecasted_winprobability'] = predictions
    
    # Save the dataframe with predictions as a parquet file
    val_df.to_parquet(file_path, index=False)
...
def main():
    ...
    try:
        ...
        pca_model_performance, cv_results, pipeline, final_estimator = apply_pca_and_cross_validation(X_train_selected, y_train, X_val_selected, y_val, best_model)

        # Output the performance
        print(f"Forecast Performance on 3 Months Validation Data Set: {pca_model_performance}")

        # Forecast using the best model on the validation set
        # Transform the validation dataset
        X_val_transformed = pipeline.transform(X_val_selected)

        val_predictions = forecast(final_estimator, X_val_transformed)  # Use X_val_selected

        # Save the forecast to a parquet file
        save_forecast_to_parquet(val_df, val_predictions, os.path.join(OUTPUT_DIR, 'forecasted_winprobability.parquet'))
